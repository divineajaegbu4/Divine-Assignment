...
import { TodosRepository } from "../todos/todo.repository.js";
...
const todoRepository = new TodosRepository(todoDB);
...
router.put("/:userID/contacts/:contactID", async (req, res) => {
  ...
  try {
    const updatedContact = await userService.updateUserContact(
      contactID,
      updatedFields,
    );
...
router.put("/:userID/todos/:todoID", async (req, res) => {
  ...
  try {
    const updatedTodo = await userService.updateUserTodo(todoID, updatedFields);
